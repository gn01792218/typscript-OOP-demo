...
function validate(validatableTnput: Validatable) {
  ...
  if (typeof value === "string" && minLength != null) {
    //對字串的檢查，!= null 就包含檢查null和undefiend
...
export abstract class RenderHTML<T extends HTMLElement, U extends HTMLElement> {
  ...
  constructor(
    ...
    private hostEle: U,
    private option: RenderOption = { insertPosition: "afterbegin" }
  ) {
  ...
  implements Draggable
{
  ...
  constructor(
    ...
    hostEle: HTMLUListElement,
    project: Project,
    _option: RenderOption = { insertPosition: "afterbegin" }
  ) {
...
export class RenderForm extends RenderHTML<HTMLElement, HTMLDivElement> {
  ...
  constructor(
    ...
    this.decsriptionInput = this.element.querySelector(
      "#description"
    ) as HTMLInputElement;
    this.propleInput = this.element.querySelector(
      "#people"
    ) as HTMLInputElement;
  ...
  implements Dropable
{
  ...
  dropHandler(event: DragEvent): void {
    ...
    //切換ProjectItem的type
    const projectState = ProjectState.getInstance();
    projectState.switchProjectType(
      projectid,
